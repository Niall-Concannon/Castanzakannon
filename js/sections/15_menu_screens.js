...
function drawEncyclopediaMenu() {
    ...
    const drawTab = (tab, active, hover, label) => {
        ...
        ctx.strokeStyle = active ? 'red' : 'rgba(210,220,235,0.5)';
        ctx.lineWidth = active ? 2 : 1;
        ctx.strokeRect(tab.x, tab.y, tab.w, tab.h);
        ctx.fillStyle = active ? 'red' : 'red';
    ...
    for (const button of filterButtons) {
        ...
        ctx.strokeStyle = active ? 'red' : 'rgba(210,220,235,0.38)';
        ctx.lineWidth = active ? 2 : 1;
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        if (icon?.complete && icon.naturalWidth) {
            ...
            ctx.fillStyle = entry.kind === 'enemy' ? 'red' : entry.kind === 'unique' ? 'red' : 'red';
...
function drawMenu() {
    ...
    if (menuPage === 'main') {
        ...
        for (let i = 0; i < cards.length; i++) {
            ...
            ctx.fillStyle = isSel ? 'red' : 'red';
            ctx.fillRect(drawX, drawY, drawW, drawH);

            ctx.strokeStyle = isSel ? 'red' : isHover ? 'red' : 'red';
...
function drawLevelUpMenu() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        ctx.save();
        if (hover) { ctx.shadowColor = 'red'; ctx.shadowBlur = 28; ctx.globalAlpha = pulse; }
        ...
        ctx.font         = 'bold 14px Arial';
        ctx.fillStyle    = hover ? 'red' : 'red';
        ctx.fillText(`Choice ${i + 1}`, c.x + c.w / 2, c.y + 78);
        ctx.font         = 'bold 16px Arial';
        ctx.fillStyle    = hover ? 'red' : rarityColor;
    ...
    ctx.save();
    if (sh) { ctx.shadowColor = 'red'; ctx.shadowBlur = 16; ctx.globalAlpha = sp; }
    ctx.drawImage(lvlSkipBgSprite, skip.x, skip.y, skip.w, skip.h);
    ctx.globalAlpha = 1;
    ctx.shadowBlur  = 0;
    ctx.textAlign   = 'center';
    ctx.font        = 'bold 16px Arial';
    ctx.fillStyle   = sh ? 'red' : 'rgba(200,200,200,0.85)';
